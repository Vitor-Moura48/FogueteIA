...
class Player(Mob):
    def __init__(self, real=False):
        ...
        dados.sprites_agentes.add(self)

        self.rede_neural: RedeNeural = None
        ...
        self.max_combustivel = 800
        self.combustivel = 800
    ...
    #@cache
    def obter_pontos(self, centro, angulo: float):
    ...
    def mover_esquerda(self, potencia: float = 1.0):
        self.velocidade_angular += (self.forca * numpy.cos(numpy.deg2rad(30))) * potencia * 0.6

    def mover_direita(self, potencia: float = 1.0):
    ...
    def obter_entradas(self):
        ...
        for sprite in dados.sprites_alvos:
            if sprite.indice == self.index_alvo:
                ...
                self.rede_neural.increment_reward( (1 - numpy.hypot(distancia_x_normalizada, distancia_y_normalizada)) * ((self.index_alvo + 1) ** 1.5) )
    ...
    def aplicar_resistencia(self):
        ...
        self.velocidade_angular -= self.velocidade_angular * self.resistencia_do_ar * 20
    
    def estabilidade_foguete(self):
        self.angulo_foquete += (90 - self.angulo_foquete) * 0.04
    ...
    def update(self):
        ...
        if not self.real:
            ...
            self.rede_neural.set_input(self.obter_entradas())
            output = self.rede_neural.get_output()

            if output[2]:
                if self.combustivel > 0:
                    self.acelerar(output[2] * 2)
                    self.index_imagem = 1
                
                    if output[0]:
                        self.mover_esquerda(output[0] * 2)
                        self.index_imagem = 2
                    if output[1]:
                        self.mover_direita(output[1] * 2)
                        self.index_imagem = 3
            else:
                self.index_imagem = 0
